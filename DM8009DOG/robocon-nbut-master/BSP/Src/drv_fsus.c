...
// 构造发送数据帧
void FSUS_SendPackage(FSUS_t *fsus, uint8_t cmdId, uint8_t size, uint8_t *content){
    // 申请内存
    ...
    FSUS_Package2RingBuffer(&pkg, fsus->sendBuf);
	// 通过串口将数据发送出去
    uint8_t value;
    while (RingBuffer_GetByteUsed(fsus->sendBuf))
    {
        value = RingBuffer_Pop(fsus->sendBuf);
        // printf("Usart_SendAll pop: %d", value);
        fsus->opt->FSUS_UART_WriteData(value);
    }
}

// 接收数据帧 (在接收的时候动态的申请内存)
FSUS_STATUS FSUS_RecvPackage(FSUS_t *fsus, PackageTypeDef *pkg){
    ...
    // 倒计时开始
    fsus->opt->FSUS_CountdownBegin(FSUS_TIMEOUT_MS);

    // 如果没有超时
    while (!fsus->opt->FSUS_CountdownIsTimeout()){
        if (RingBuffer_GetByteUsed(fsus->recvBuf) == 0){
			// 没有新的字节读进来, 继续等待
            continue;
        }
        ...
        if(pkg->status & FSUS_RECV_FLAG_CONTENT){
            // 参数内容接收完成
            // 接收校验码
            pkg->checksum = RingBuffer_ReadByte(fsus->recvBuf);
            // 设置状态码-校验码已经接收
            pkg->status = pkg->status | FSUS_RECV_FLAG_CHECKSUM;
            // 直接进行校验码检查
            if (FSUS_CalcChecksum(pkg) != pkg->checksum){
                // 撤销倒计时
                fsus->opt->FSUS_CountdownCancel();
                // 校验码错误
                return FSUS_STATUS_CHECKSUM_ERROR;
            }else{
                // 撤销倒计时
            ...
            // Size已经接收完成
            // 接收参数字节
            pkg->content[bIdx] = RingBuffer_ReadByte(fsus->recvBuf);
            bIdx ++;
            // 判断是否接收完成
            if (bIdx == pkg->size){
                // 标记为参数接收完成
            ...
            // 指令接收完成
            // 接收尺寸信息
            pkg->size = RingBuffer_ReadByte(fsus->recvBuf);
            // 判断长度size是否合法
            // 参数的size大于FSUS_PACK_RESPONSE_MAX_SIZE里面的限制
            if (pkg->size > (FSUS_PACK_RESPONSE_MAX_SIZE - 5)){
                // 撤销倒计时
            ...
            // 帧头已接收 
            // 接收指令ID
            pkg->cmdId = RingBuffer_ReadByte(fsus->recvBuf);
            // 判断指令是否合法
            // 判断控制指令是否有效 指令范围超出
            if (pkg->cmdId > FSUS_CMD_NUM){
                // 撤销倒计时
            ...
            if (header == 0){
                // 接收第一个字节
                header = RingBuffer_ReadByte(fsus->recvBuf);
                // 判断接收的第一个字节对不对
                ...
                // 接收帧头第二个字节
                header =  header | (RingBuffer_ReadByte(fsus->recvBuf) << 8);
				// 检查第二个字节是否正确
...
FSUS_STATUS FSUS_Ping(FSUS_t *fsus, uint8_t servo_id){
	uint8_t statusCode; // 状态码
	uint8_t ehcoServoId; // PING得到的舵机ID
	// printf("[PING]Send Ping Package\r\n");
	// 发送请求包
	FSUS_SendPackage(fsus, FSUS_CMD_PING, 1, &servo_id);
	// 接收返回的Ping
	PackageTypeDef pkg;
	statusCode = FSUS_RecvPackage(fsus, &pkg);
...
// 重置舵机的用户资料
FSUS_STATUS FSUS_ResetUserData(FSUS_t *fsus, uint8_t servo_id){
	const uint8_t size = 1;
	FSUS_STATUS statusCode;
	// 发送请求包
	FSUS_SendPackage(fsus, FSUS_CMD_RESET_USER_DATA, size, &servo_id);
	// 接收重置结果
	PackageTypeDef pkg;
	statusCode = FSUS_RecvPackage(fsus, &pkg);
	if (statusCode == FSUS_STATUS_SUCCESS){
...
// 读取数据
FSUS_STATUS FSUS_ReadData(FSUS_t *fsus, uint8_t servo_id,  uint8_t address, uint8_t *value, uint8_t *size){
	FSUS_STATUS statusCode;
	// 构造content
	uint8_t buffer[2] = {servo_id, address};
	// 发送请求数据
	FSUS_SendPackage(fsus, FSUS_CMD_READ_DATA, 2, buffer);
	// 接收返回信息
	PackageTypeDef pkg;
	statusCode = FSUS_RecvPackage(fsus, &pkg);
	if (statusCode == FSUS_STATUS_SUCCESS){
...
#if 0
// 写入数据
FSUS_STATUS FSUS_WriteData(FSUS_t *fsus, uint8_t servo_id, uint8_t address, uint8_t *value, uint8_t size){
	FSUS_STATUS statusCode;
	// 构造content
	...
	// 发送请求数据
	FSUS_SendPackage(fsus, FSUS_CMD_WRITE_DATA, size+2, buffer);
	// 接收返回信息
	PackageTypeDef pkg;
	statusCode = FSUS_RecvPackage(fsus, &pkg);
	if (statusCode == FSUS_STATUS_SUCCESS){
 ...
 * speed单位 °/s
 */
FSUS_STATUS FSUS_WheelMove(FSUS_t *fsus, uint8_t servo_id, uint8_t method, uint16_t speed, uint16_t value){
	// 创建环形缓冲队列
	const uint8_t size = 6;
	uint8_t buffer[7];//size+1
	RingBufferTypeDef ringBuf;
	...
	// 发送请求包
	FSUS_SendPackage(fsus, FSUS_CMD_SPIN, size, buffer+1);
	
	return FSUS_STATUS_SUCCESS;
}

// 轮式模式, 舵机停止转动
FSUS_STATUS FSUS_WheelStop(FSUS_t *fsus, uint8_t servo_id){
	uint8_t method = 0x00;
	uint16_t speed = 0;
	uint16_t value = 0;
	return FSUS_WheelMove(fsus, servo_id, method, speed, value);
}

// 轮式模式 不停的旋转
FSUS_STATUS FSUS_WheelKeepMove(FSUS_t *fsus, uint8_t servo_id, uint8_t is_cw, uint16_t speed){
...
// 轮式模式 按照特定的速度旋转特定的时间
FSUS_STATUS FSUS_WheelMoveTime(FSUS_t *fsus, uint8_t servo_id, uint8_t is_cw, uint16_t speed, uint16_t nTime){
	uint8_t method = 0x03; // 旋转一段时间
	if (is_cw){
		// 顺时针旋转
		method = method | 0x80;
	}
	return FSUS_WheelMove(fsus, servo_id, method, speed, nTime);
}

// 轮式模式 旋转特定的圈数
FSUS_STATUS FSUS_WheelMoveNCircle(FSUS_t *fsus, uint8_t servo_id, uint8_t is_cw, uint16_t speed, uint16_t nCircle){
	uint8_t method = 0x02; // 旋转特定的圈数
	if (is_cw){
		// 顺时针旋转
		method = method | 0x80;
	}
	return FSUS_WheelMove(fsus, servo_id, method, speed, nCircle);
}
...
// @power 舵机执行功率 单位mW
//        若power=0或者大于保护值
FSUS_STATUS FSUS_SetServoAngle(FSUS_t *fsus, uint8_t servo_id, float angle, uint16_t interval, uint16_t power, uint8_t wait){
	// 创建环形缓冲队列
	const uint8_t size = 7;
	uint8_t buffer[8];//size+1
	RingBufferTypeDef ringBuf;
	RingBuffer_Init(&ringBuf, size, buffer);	
	// 数值约束
	if(angle > 180.0f){
	...
	FSUS_SendPackage(fsus, FSUS_CMD_ROTATE, size, buffer+1);
	
	if (wait){
		return FSUS_Wait(fsus, servo_id, angle, 0); 
	}else{
		return FSUS_STATUS_SUCCESS;
	}
}
...
FSUS_STATUS FSUS_SetServoAngleByInterval(FSUS_t *fsus, uint8_t servo_id, \
	...
	// 发送请求包
	// 注: 因为用的是环形队列 head是空出来的,所以指针需要向后推移一个字节
	FSUS_SendPackage(fsus, FSUS_CMD_SET_SERVO_ANGLE_BY_INTERVAL, size, buffer+1);
...
/* 设置舵机的角度(指定转速) */
FSUS_STATUS FSUS_SetServoAngleByVelocity(FSUS_t *fsus, uint8_t servo_id, \
	...
	// 发送请求包
	// 注: 因为用的是环形队列 head是空出来的,所以指针需要向后推移一个字节
	FSUS_SendPackage(fsus, FSUS_CMD_SET_SERVO_ANGLE_BY_VELOCITY, size, buffer+1);
...
FSUS_STATUS FSUS_QueryServoAngle(FSUS_t *fsus, uint8_t servo_id, float *angle){
	const uint8_t size = 1; // 请求包content的长度
	uint8_t ehcoServoId;
	int16_t echoAngle;
	
	// 发送舵机角度请求包
	FSUS_SendPackage(fsus, FSUS_CMD_READ_ANGLE, size, &servo_id);
	// 接收返回的Ping
	PackageTypeDef pkg;
	uint8_t statusCode = FSUS_RecvPackage(fsus, &pkg);
	if (statusCode == FSUS_STATUS_SUCCESS){
		// 成功的获取到舵机角度回读数据
		ehcoServoId = (uint8_t)pkg.content[0];
		// 检测舵机ID是否匹配
...
/* 设置舵机的角度(多圈模式) */
FSUS_STATUS FSUS_SetServoAngleMTurn(FSUS_t *fsus, uint8_t servo_id, float angle, \
	...
	// 发送请求包
	// 注: 因为用的是环形队列 head是空出来的,所以指针需要向后推移一个字节
	FSUS_SendPackage(fsus, FSUS_CMD_SET_SERVO_ANGLE_MTURN, size, buffer+1);
	
	if(wait){
		return FSUS_Wait(fsus, servo_id, angle, 1);
	}else{
		return FSUS_STATUS_SUCCESS;
	}
}
...
/* 设置舵机的角度(多圈模式, 指定周期) */
FSUS_STATUS FSUS_SetServoAngleMTurnByInterval(FSUS_t *fsus, uint8_t servo_id, float angle, \
			uint32_t interval,  uint16_t t_acc,  uint16_t t_dec, uint16_t power, uint8_t wait){
	// 创建环形缓冲队列
	const uint8_t size = 15;
	uint8_t buffer[16];//size+1
	RingBufferTypeDef ringBuf;
	RingBuffer_Init(&ringBuf, size, buffer);	
	...
	// 发送请求包
	// 注: 因为用的是环形队列 head是空出来的,所以指针需要向后推移一个字节
	FSUS_SendPackage(fsus, FSUS_CMD_SET_SERVO_ANGLE_MTURN_BY_INTERVAL, size, buffer+1);
...
/* 设置舵机的角度(多圈模式, 指定转速) */
FSUS_STATUS FSUS_SetServoAngleMTurnByVelocity(FSUS_t *fsus, uint8_t servo_id, float angle, \
			float velocity, uint16_t t_acc,  uint16_t t_dec, uint16_t power, uint8_t wait){
	// 创建环形缓冲队列
	const uint8_t size = 13;
	uint8_t buffer[14];//size+1
	RingBufferTypeDef ringBuf;
	RingBuffer_Init(&ringBuf, size, buffer);	
	// 数值约束
	if(angle > 368640.0f){
	...
	// 发送请求包
	// 注: 因为用的是环形队列 head是空出来的,所以指针需要向后推移一个字节
	FSUS_SendPackage(fsus, FSUS_CMD_SET_SERVO_ANGLE_MTURN_BY_VELOCITY, size, buffer+1);
...
/* 查询舵机的角度(多圈模式) */
FSUS_STATUS FSUS_QueryServoAngleMTurn(FSUS_t *fsus, uint8_t servo_id, float *angle){
	// 创建环形缓冲队列
	const uint8_t size = 1; // 请求包content的长度
	uint8_t ehcoServoId;
	int32_t echoAngle;
	...
	// 发送舵机角度请求包
	FSUS_SendPackage(fsus, FSUS_CMD_QUERY_SERVO_ANGLE_MTURN, size, &servo_id);
	// 接收返回的Ping
	PackageTypeDef pkg;
	uint8_t statusCode = FSUS_RecvPackage(fsus, &pkg);
	if (statusCode == FSUS_STATUS_SUCCESS){
...
/* 舵机阻尼模式 */
FSUS_STATUS FSUS_DampingMode(FSUS_t *fsus, uint8_t servo_id, uint16_t power){
	const uint8_t size = 3; // 请求包content的长度
	uint8_t buffer[4]; // content缓冲区 size+1
	...
	// 发送请求包
	// 注: 因为用的是环形队列 head是空出来的,所以指针需要向后推移一个字节
	FSUS_SendPackage(fsus, FSUS_CMD_DAMPING, size, buffer+1);
	return FSUS_STATUS_SUCCESS;
}


/* 等待电机旋转到特定的位置 */
FSUS_STATUS FSUS_Wait(FSUS_t *fsus, uint8_t servo_id, float target_angle, uint8_t is_mturn){
	...
	while(1){
		if(is_mturn){
			 FSUS_QueryServoAngleMTurn(fsus, servo_id, &angle_read);
		}else{
			 FSUS_QueryServoAngle(fsus, servo_id, &angle_read);
		}
		...
		// 延时100ms
		fsus->opt->FSUS_Delay_Ms(100);
	}
	
}

void FSUS_GetData_IRQHandler(FSUS_t *fsus, uint8_t data)
{
	uint8_t ucTemp;
	ucTemp=fsus->opt->FSUS_UART_ReadData();
	RingBuffer_Push(fsus->recvBuf, ucTemp);
}
