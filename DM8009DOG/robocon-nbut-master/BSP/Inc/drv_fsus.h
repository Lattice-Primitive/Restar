...
// 静止状态判断条件
#define FSUS_ANGLE_DEADAREA 1.5f // 电机角度死区
...
typedef struct
{
    void (*FSUS_UART_WriteData)(uint8_t data);
    uint8_t (*FSUS_UART_ReadData)(void);
    void (*FSUS_Delay_Ms)(uint32_t ticks);
    void (*FSUS_CountdownBegin)(uint32_t nTime);
    void (*FSUS_CountdownCancel)(void);
    uint8_t (*FSUS_CountdownIsTimeout)(void);
}FSUS_Operation_t;

typedef struct 
{
    FSUS_Operation_t *opt;
    // 发送端缓冲区
