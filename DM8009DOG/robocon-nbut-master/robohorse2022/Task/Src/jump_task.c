...
void Jump_OnlyOnce()
{
    const float prep_time = 0.8f;   // 准备时间 [s]		0.8 准备动作
    const float launch_time = 0.2f; // 收缩腿前的持续时间 [s]		0.2  跳跃过程收缩
    ...
    if (NowState == JUMP_GROUND)
    {
        ...
        while (t <= prep_time + launch_time + fall_time + stabilize_time)
            ...
            if (t < prep_time)
            {
							  for(int a=0;a<8;a++)
							{
               PID_Reset(&PID_Position[a], 100.0f, 0.0008f, 1.5f);
							}
//							  PID_Reset(&PID_Position[a], 3.0f, 0.0008f, 0.0f);
            ...
            else if (t >= prep_time && t < prep_time + launch_time)
            {
                ...
                {
                    Action_Polar[i].radius = 24.0f;//28
                    Action_Polar[i].theta = -20.0f;
                }
      for(int a=0;a<8;a++)
							{ 
								PID_Reset(&PID_Position[a], 100.0f, 0.0008f, 2.0f);
							}
//                PID_Reset(PID_Position, 22.0f, 0.0008f, 0.0f);
            ...
            else if (t >= prep_time + launch_time && t < prep_time + launch_time + fall_time)
            {
                for (uint8_t i = 0; i < 4; i++)
                {
                    ...
                    Action_Polar[i].theta = 13.0f;
                }
 for(int a=0;a<8;a++)
							{
               PID_Reset(&PID_Position[a], 100.0f, 0.0008f, 3.0f);
							}
//                PID_Reset(PID_Position, 15.0f, 0.0008f, 0.0f);
 ...
 for(int a=0;a<8;a++)
							{
                 PID_Reset(&PID_Position[a], 60.0f, 0.0008f, 1.0f);
							}
//                PID_Reset(PID_Position, 15.0f, 0.0008f, 0.0f);
            ...
            else if (t > prep_time + launch_time + fall_time + stabilize_time)
            {
                Change_NowState(STOP);
							 for(int a=0;a<8;a++)
							{
                PID_Reset(&PID_Position[a], 80.0f, 0.0008f, 2.0f);
							}
//                PID_Reset(PID_Position, 3.0f, 0.0008f, 0.1f);
...
//                PID_Reset(PID_Position, 5.0f, 0.0008f, 0.0f);
 ...
 for(int a=0;a<8;a++)
							{
               PID_Reset(&PID_Position[a], 60.0f, 0.0008f, 2.0f);
							}
...
//                PID_Reset(PID_Position, 8.0f, 0.0008f, 0.0f);
 for(int a=0;a<8;a++)
							{
               PID_Reset(&PID_Position[a], 40.0f, 0.0008f, 2.0f);
							}
...
//        Aim_Angle[i * 2 + 1] = -(pi / 2.0f + Gait_Data[i].angle[0]) / 1.0f;
        if (i == 0) { // 左前腿
            //左侧
            Aim_Angle[2] = (pi / 2.0f - Gait_Data[i].angle[1]) / 1.0f;    
            //右侧
            Aim_Angle[3] = (pi / 2.0f + Gait_Data[i].angle[0]) / 1.0f;    
        }
        else if (i == 1) { // 左前腿
         //左侧
            Aim_Angle[5] = -(pi / 2.0f - Gait_Data[i].angle[1]) / 1.0f;     //外侧电机
            //右侧
            Aim_Angle[4] = -(pi / 2.0f + Gait_Data[i].angle[0]) / 1.0f;     //内侧电机
         ...
         //左侧
            Aim_Angle[7] = (pi / 2.0f - Gait_Data[i].angle[1]) / 1.0f;     //外侧电机
            //右侧
            Aim_Angle[6] = (pi / 2.0f + Gait_Data[i].angle[0]) / 1.0f;     //内侧电机
           
        }
        else if (i == 3) { // 左前腿
           //左侧
            Aim_Angle[1] = -(pi / 2.0f - Gait_Data[i].angle[1]) / 1.0f;     
            //右侧
            Aim_Angle[0] = -(pi / 2.0f + Gait_Data[i].angle[0]) / 1.0f;     //内侧电机
        }
		}
